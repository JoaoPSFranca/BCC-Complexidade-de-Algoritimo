...
def run_bruteforce(edges, graph_id, print_details=True):
    ...
    if print_details:
        ...
        draw_graph(G, graph_id, "BruteForce", "BruteForce", cycle, edge_color='red')
...
def run_heldkarp(edges, graph_id, print_details=True):
    ...
    if print_details:
        ...
        for i, row in enumerate(tsp_matrix):
            row_str = "  ".join(f"{val if val != float('inf') else '-':>3}" for val in row)
            print(f"{nodes[i]}  {row_str}")
        draw_graph(G, graph_id, "HeldKarp", "Held-Karp", [nodes[i] for i in path] if is_ham else None, edge_color='red')
...
def run_linkernighan(edges, graph_id, print_details=True):
    ...
    if print_details:
        ...
        draw_graph(G, graph_id, "LinKernighan", "Lin-Kernighan", cycle, edge_color='green')
